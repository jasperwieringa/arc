...
describe('ArcButton', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders the button with default properties in the dom', () => {
      expect(element).dom.to.equal(
        `<arc-button type='${BUTTON_TYPES.filled}' color='${THEME_PALETTE.primary}' size='${INPUT_SIZES.medium}'>Test</arc-button>`
      );
    });
  ...
  describe('setters/getters', () => {
    it('renders the button with a custom color, type and size property', async () => {
      ...
      for (const buttonColor of Object.values(THEME_PALETTE)) {
