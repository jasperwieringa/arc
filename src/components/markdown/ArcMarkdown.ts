...
export default class ArcMarkdown extends LitElement {
  ...
  */
  ...
  private _getOutput(): string {
    /* Regular expression to find <p><br></p>. */
    const regex = /<p\s*\/?><br\s*\/?><\/p\s*\/?>/gi;

    switch (this.output) {
      case OUTPUT_TYPES.html:
        /*
        Quill adds additional <br> tags within empty <p> tags.
        Replace <p><br></p> with <p></p> using the regex.
        */
        return this._editor.querySelector('.ql-editor')?.innerHTML.replace(regex, '<p></p>');
